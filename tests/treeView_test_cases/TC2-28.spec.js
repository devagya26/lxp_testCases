...
test("LXP treeView_testCase", async ({ page }) => {
  ...
  await iframe.locator("id=1").click();
  await iframe.locator("id=2").click();
  await iframe.locator("id=10000").click();
  await iframe.getByRole("button", { name: "Priority" }).click();

  await iframe.getByRole("button", { name: "Issue Link Type" }).click();
  await iframe.getByRole("button", { name: "Clear All" }).click();
  await iframe.locator("id=PARENT").click();
  await iframe.locator("id=10003-relates to").click();
  await iframe.getByRole("button", { name: "Issue Link Type" }).click();

  await iframe.getByRole("button", { name: "Issue Type" }).click();
  await iframe.getByRole("button", { name: "Clear All" }).click();
  await iframe.locator("id=subtask").click();
  await iframe.locator("id=story").click();
  await iframe.getByRole("button", { name: "Issue Type" }).click();

  await iframe.getByRole("button", { name: "Issue Card Fields" }).click();
  await iframe.getByRole("button", { name: "Clear All" }).click();
  await iframe.locator("id=issueType").click();
  await iframe.locator("id=assignee").click();
  await iframe.locator("id=description").click();
  await iframe.locator("id=summary").click();
  await iframe.getByRole("button", { name: "Issue Card Fields" }).click();
  ...
  (() => {
    const file1 = fs.readFileSync(
      "/Users/apple/Desktop/PlaywrightAutomation/expectedTC2/TC2-28.json"
    );
