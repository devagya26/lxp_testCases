...
test("LXP treeView_testCase", async ({ page }) => {
  ...
  const checkbox1 = await iframe.locator("id=PARENT");
  await checkbox1.check();
  await expect(checkbox1).toBeChecked();

  const checkbox2 = await iframe.locator("id=SUBTASKS");
  ...
  (() => {
    const file1 = fs.readFileSync(
      "/Users/apple/Desktop/PlaywrightAutomation/expectedTC2/TC2-15.json"
    );
